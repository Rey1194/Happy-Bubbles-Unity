...
#endif
{
	...
	private static class API
	{
		...
		public static MethodInfo addTabMethod2;
	...
	//	public static EditorApplication.CallbackFunction windowsReordered;
		...
		static API()
		{
			...
			if (dockAreaType != null)
			{
				...
				addTabMethod2 = dockAreaType.GetMethod("AddTab", new System.Type[] { typeof(EditorWindow), typeof(bool) });
			}
	...
	private static void OnProjectItemGUI(string item, Rect selectionRect)
	{
		...
		if (path.EndsWithDll() || path.EndsWithExe())
		{
			return;
		}
		...
		if (!path.EndsWithCS() && !path.EndsWithJS() && !path.EndsWithBoo())
		{
			// not a script
	...
	[MenuItem("CONTEXT/MonoBehaviour/Edit in Script Inspector", true, 610)]
	private static bool CanOpenBehaviourScript(MenuCommand mc)
	{
		MonoBehaviour target = mc.context as MonoBehaviour;
		if (target == null)
			return false;
	...
	{
		var target = mc.context as Component;
		if (target == null)
			return;
		var currentEditor = ActiveEditorTracker.sharedTracker.activeEditors.FirstOrDefault(x => x.target == target);
		if (currentEditor == null)
			return;
		var monoScript = MonoScript.FromScriptableObject(currentEditor);
		if (monoScript == null)
			return;
		string scriptPath = AssetDatabase.GetAssetPath(monoScript);
		if (string.IsNullOrEmpty(scriptPath))
	...
	public static bool OnOpenAsset(int instanceID, int line)
	{
		...
		if (asset is MonoScript)
		{
			...
			if (assetPath.EndsWithDll() || assetPath.EndsWithExe())
			{
				return false;
			}
		}
	...
	{
		if (API.windowsField == null || API.mainViewField == null || API.panesField == null || API.addTabMethod == null && API.addTabMethod2 == null)
			return false;
		...
		if (API.addTabMethod != null)
			API.addTabMethod.Invoke(addToView, new object[] { dockThis });
		else
			API.addTabMethod2.Invoke(addToView, new object[] { dockThis, true });
		return true;
	}
	...
	[System.NonSerialized]
	public bool ignoreNextLostFocusEvent = false;
	public void OnLostFocus()
	{
		if (ignoreNextLostFocusEvent)
		{
			ignoreNextLostFocusEvent = false;
			return;
		}
			...
			case EventType.DragPerform:
				if (DragAndDrop.objectReferences.Length > 0)
				{
					...
					foreach (Object obj in DragAndDrop.objectReferences)
					{
						...
						if (obj is GameObject)
						{
							...
							foreach (MonoBehaviour mb in monoBehaviours)
							{
								...
								if (monoScript != null)
								{
									...
									if (!string.IsNullOrEmpty(assetPath) &&
										assetPath.StartsWithIgnoreCase("assets/") &&
										!assetPath.EndsWithDll())
									{
						...
						else if (assetPath.StartsWithIgnoreCase("assets/"))
						{
...
#else
		if (titleContent.text.FastStartsWith("*"))
		{
			if (!isModified)
	...
	public void AddItemsToMenu(GenericMenu menu)
	{
		if (!string.IsNullOrEmpty(targetAssetGuid))
		{
			...
			menu.AddItem(new GUIContent("Copy as Path"), false, () => {
				var path = System.IO.Path.GetFullPath(assetPath);
				if (path.IndexOf(' ') >= 0)
			...
			menu.AddItem(new GUIContent("Print with Notepad++"), false, () => {
				EditorApplication.ExecuteMenuItem("Assets/Print with Notepad++");
			});
#endif
			menu.AddSeparator("");
			...
			else
			{
				...
				menu.AddItem("Close SI Tabs to the Right", "", "Close SI Tabs to the Right", "", false, CloseTabsRightOf);
			}
	...
	[MenuItem("Window/Script Inspector 3/Reset SI Tabs Position", false, 812)]
	public static void ResetSITabsPosition()
	{
		CloseAllSITabs();
		
		defaultDockNextTo = null;
		defaultPosition = new Rect(100f, 100f, 600f, 380f);
		SaveDefaultPosition();
		EditorPrefs.SetString("ScriptInspectorDefaultDockNextTo", "");
	}
	
	[MenuItem("Window/Script Inspector 3/Close All SI Tabs", true, 811)]
	public static bool ValidateCloseAllSITabs()
	{
		return codeWindows.Count > 0;
	}
	
	[MenuItem("Window/Script Inspector 3/Close All SI Tabs", false, 811)]
	public static void CloseAllSITabs()
	{
	...
	public void CloseTabsRightOf()
	{
		...
		for (var i = panes.Count - 1; i > index; --i)
		{
			var asCodeWindow = panes[i] as FGCodeWindow;
			if (asCodeWindow == null)
				continue;
			asCodeWindow.Close();
		}
