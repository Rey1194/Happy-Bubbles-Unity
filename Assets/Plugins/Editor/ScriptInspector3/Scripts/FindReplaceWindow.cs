...
public class FindReplaceWindow : EditorWindow
{
	...
	private static char[] excludeWithCharacters = new []{'/'};
#else
	private static char[] excludeWithCharacters = new []{'_', '/'};
#endif
	...
	private void ShowFindHistory()
	{
		...
		if (findText != initialFindText && initialFindText.Trim() != "" && initialFindText.IndexOfAny(excludeWithCharacters) < 0)
		...
		for (var i = 0; i < searchHistory.Length && searchHistory[i] != null; i++)
			if (findText != searchHistory[i] && searchHistory[i].IndexOfAny(excludeWithCharacters) < 0)
	...
	private void ShowReplaceHistory()
	{
...
#endif
		GenericMenu menu = new GenericMenu();
		if (replaceText != initialFindText && initialFindText.Trim() != "" && initialFindText.IndexOfAny(excludeWithCharacters) < 0)
		...
		for (var i = 0; i < replaceHistory.Length && replaceHistory[i] != null; i++)
			if (replaceText != replaceHistory[i] && replaceHistory[i].IndexOfAny(excludeWithCharacters) < 0)
...
#else
			menu.DropDown(new Rect(14f, 104f, EditorGUIUtility.currentViewWidth - 10f, 18f));
#endif
		Event.current.Use();
	}
