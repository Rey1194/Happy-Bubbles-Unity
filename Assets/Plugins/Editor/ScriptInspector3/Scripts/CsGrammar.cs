...
public class CsGrammar : FGGrammar
{
	...
	//	parser = new Parser(r_compilationUnit, this);
		...
		if (!CsParser.isCSharp4)
		{
			parser.Add(new Rule("classMemberDeclaration",
				new Seq(
					...
					| new If("async", ASYNC - modifiers - PARTIAL - "void", ASYNC - modifiers - PARTIAL - "void" - methodDeclaration)
					| new If("async", ASYNC - modifiers - "void", ASYNC - modifiers - "void" - methodDeclaration)
					| new If("async", ASYNC - modifiers - PARTIAL - type - memberName - "(", ASYNC - modifiers - PARTIAL - type - methodDeclaration)
		...
		var throwExpression = new Id("throwExpression");
...
//	| 'struct' ;
		...
		else
		{
			parser.Add(new Rule("methodBody",
				"{" - statementList - "}"
				| "=>" - (throwExpression | new Opt("ref") - expression) - ";"
				| ";"
			) { semantics = SemanticFlags.MethodBodyScope });
		}
	...
	//	var declarationStatement = new Id("declarationStatement");
		...
		//var awaitStatement = new Id("awaitStatement");
		var awaitExpression = new Id("awaitExpression");
	...
	//		));
		...
		var readonlyAccessorDeclaration = new Id("readonlyAccessorDeclaration");
		var expressionBodiedGetAccessorBody = new Id("expressionBodiedGetAccessorBody");
		...
		{
			parser.Add(new Rule("indexerDeclaration",
				new Lit("this") - "[" - formalParameterList - "]" -
				(
					("{" - accessorDeclarations - "}")
					| readonlyAccessorDeclaration
				)
			) { semantics = SemanticFlags.IndexerDeclaration | SemanticFlags.AccessorsListScope });
		}
		...
		else
		{
			parser.Add(new Rule("propertyDeclaration",
				memberName - (
					readonlyAccessorDeclaration
					| "{" - accessorDeclarations - "}" - new Opt("=" - expression - ";")
				)
			) { semantics = SemanticFlags.PropertyDeclaration | SemanticFlags.AccessorsListScope });
		}
		...
		parser.Add(new Rule("operatorDeclarator",
			...
			) { semantics = SemanticFlags.OperatorDeclarator | SemanticFlags.MethodDeclarationScope });
...
//				| IDENTIFIER
		...
		if (!CsParser.isCSharp4)
		{
			var AWAIT = new Id("AWAIT");
			parser.Add(new Rule("AWAIT",
				"await" | IDENTIFIER
			){ contextualKeyword = true });

			parser.Add(new Rule("awaitExpression",
		...
		var NAMEOF = new Id("NAMEOF");
		var nameofExpression = new Id("nameofExpression");
		if (!CsParser.isCSharp4)
		{
			parser.Add(new Rule("NAMEOF",
				new If(s => s.Current.text == "nameof", IDENTIFIER | "nameof")
			){ contextualKeyword = true });
		
			parser.Add(new Rule("nameofExpression",
				NAMEOF - "(" - expression - ")"
			));
		}
...
//*
		...
		(new Lit(".") | "?.") - IDENTIFIER - new If(typeArgumentList, typeArgumentList)
			));
		...
		else
		{
			...
			parser.Add(new Rule("argumentValue",
				...
				| new Seq( new Lit("out") - new Opt("var") | "ref" | "in", variableReference )
				| ".EXPECTEDTYPE"
				));
		}
		...
		var ASCENDING_OR_DESCENDING = new Id("ASCENDING_OR_DESCENDING"); parser.Add(new Rule("ASCENDING_OR_DESCENDING", IDENTIFIER | "ascending" | "descending") { contextualKeyword = true });
		...
		parser.Add(new Rule("ordering",
			expression
			- new Opt(new If(s => s.Current.text == "ascending" || s.Current.text == "descending", ASCENDING_OR_DESCENDING))
			));
		...
		{
			parser.Add(new Rule("throwExpression",
				"throw" - expression
				));
			
			parser.Add(new Rule("conditionalExpression",
				nullCoalescingExpression - new Opt("?" - (
					("ref" - (expression | throwExpression) - ":" - "ref" - (expression | throwExpression))
					| ((expression | throwExpression) - ":" - (expression | throwExpression))
				))) { autoExclude = true });
		}
		...
		{
			parser.Add(new Rule("nullCoalescingExpression",
				conditionalOrExpression - new Many("??" - (throwExpression | conditionalOrExpression))
				) { autoExclude = true });
		}
	...
	public override IdentifierCompletionsType GetCompletionTypes(ParseTree.BaseNode afterNode)
	{
		...
		if (leaf.token.text == ".")
			...
			cit = IdentifierCompletionsType.Member;
	...
	public static ParseTree.Node EnclosingScopeNode(ParseTree.Node node, SemanticFlags scopeType)
	{
		while (node != null)
		{
			var scopeSemantics = node.semantics & SemanticFlags.ScopesMask;
			if (scopeSemantics != SemanticFlags.None)
				if (scopeType == scopeSemantics)
					return node;
			node = node.parent;
		}
		return null;
	...
	public static ParseTree.Node EnclosingScopeNode(ParseTree.Node node, SemanticFlags scopeType1, SemanticFlags scopeType2)
	{
		while (node != null)
		{
			var scopeSemantics = node.semantics & SemanticFlags.ScopesMask;
			if (scopeSemantics != SemanticFlags.None)
				if (scopeType1 == scopeSemantics || scopeType2 == scopeSemantics)
					return node;
			node = node.parent;
		}
		return null;
	...
	public static ParseTree.Node EnclosingScopeNode(ParseTree.Node node, SemanticFlags scopeType1, SemanticFlags scopeType2, SemanticFlags scopeType3)
	{
		while (node != null)
		{
			var scopeSemantics = node.semantics & SemanticFlags.ScopesMask;
			if (scopeSemantics != SemanticFlags.None)
				if (scopeType1 == scopeSemantics || scopeType2 == scopeSemantics || scopeType3 == scopeSemantics)
					return node;
			node = node.parent;
		}
		return null;
			...
			switch (scopeSemantics)
			{
				...
				case SemanticFlags.NamespaceBodyScope:
					...
					node.scope = new NamespaceScope(node)
					{
						...
						definition = declaration != null ? (NamespaceDefinition) declaration.definition : null,
	...
	private static SymbolDeclaration GetNodeDeclaration(ParseTree.Node node, string fileName = null)
	{
		if (node.declaration == null)
		{
			...
			ParseTree.BaseNode asyncNode = null;
			ParseTree.Node modifiersNode2 = null;
			...
			switch (declarationSemantics)
			{
				...
				case SemanticFlags.MethodDeclarator:
					...
					if (!CsParser.isCSharp4)
					{
						asyncNode = node.parent.FindChildByName("ASYNC");
						if (asyncNode != null && modifiersNode != null)
						{
							modifiersNode2 = asyncNode.nextSibling as ParseTree.Node;
							if (modifiersNode2 != null && modifiersNode2.RuleName != "modifiers")
								modifiersNode2 = null;
						}
					}
					partialNode = node.parent.FindChildByName("PARTIAL");
			...
			if (node.declaration != null)
			{
				...
				if (asyncNode != null)
				{
					node.declaration.modifiers |= Modifiers.Async;
					if (modifiersNode2 != null)
						node.declaration.modifiers |= ParseModifiers(modifiersNode2);
				}
				if (typeParamsNode != null)
				...
				else
				{
					...
					try
					{
						var symbol = enclosingScope.AddDeclaration(node.declaration);
						if (symbol != null && typeParamsNode != null)
						{
							enclosingScopeNode = EnclosingSemanticNode(typeParamsNode.parent, SemanticFlags.ScopesMask);
							enclosingScope = GetNodeScope(enclosingScopeNode, fileName);
							for (var t = 2; t < typeParamsNode.numValidNodes; t += 3)
							{
								var tpNode = typeParamsNode.NodeAt(t);
								if (tpNode == null)
									continue;
								var tpSymbol = GetNodeDeclaration(tpNode, fileName);
								if (tpSymbol != null)
								{
									tpSymbol.scope = enclosingScope;
									symbol.AddDeclaration(tpSymbol);
								}
							}
						}
						//if (symbol != null)
	...
	public void OnReduceSemanticNode(ParseTree.Node node, string fileName = null)
	{
		...
		else if (node.RuleName != "typeParameter")
		{
	...
	private bool IsAwaitInsideAsyncMethod(IScanner s)
	{
		if (CsParser.isCSharp4)
			return false;
			
		if (s.Current.text != "await")
			return false;
		var methodDeclarationNode = s.CurrentParseTreeNode.FindParentByName("methodDeclaration");
		if (methodDeclarationNode == null)
			return false;
		if (methodDeclarationNode.declaration == null)
			return methodDeclarationNode.parent.FindChildByName("ASYNC") != null;
		return methodDeclarationNode.declaration.IsAsync;
	}
	...
	public class Scanner : IScanner
	{
		...
		public override IScanner Clone()
		{
		...
		public override void OnReduceSemanticNode(ParseTree.Node node)
		{
		...
		public override void CollectCompletions(TokenSet tokenSet)
		{
		...
		public override void InsertMissingToken(ErrorMessageProvider errorMessage)
		{
