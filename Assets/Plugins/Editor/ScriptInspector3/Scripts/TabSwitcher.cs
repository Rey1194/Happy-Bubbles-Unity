...
public class TabSwitcher : FGPopupWindow
{
	...
	public void OnGUI()
	{
		...
		{
			Event.current.Use();
			var pos = Event.current.mousePosition - new Vector2(4f, captionHeight);
			if (pos.x >= 0f && pos.y >= 0f && pos.y < itemHeight * 10f)
			{
				var newSelected = (int)(pos.y / itemHeight) + 10 * (int)(pos.x / itemsWidth);
				if (newSelected != selected)
				{
					selected = newSelected;
					Repaint();
