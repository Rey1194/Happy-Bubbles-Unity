...
//		} catch {}
	...
	public delegate void InsertedTextAllDelegate(string assetGuid, CaretPos from, CaretPos to);
	...
	private void NotifyInsertedText(CaretPos from, CaretPos to)
	{
		...
		if (onInsertedTextAll != null)
			onInsertedTextAll(guid, from.Clone(), to.Clone());
	}
	...
	public delegate void RemovedTextAllDelegate(string assetGuid, CaretPos from, CaretPos to);
	...
	private void NotifyRemovedText(CaretPos from, CaretPos to)
	{
		...
		if (onRemovedTextAll != null)
			onRemovedTextAll(guid, from.Clone(), to.Clone());
	}
	...
	private int UpdateLexer(int fromLine, int toLineInclusive, bool keepLastChangeId)
	{
		...
		while (line < formatedLines.Length)
		{
			...
			if ((parser == null || !parser.scriptDefinesChanged) && prevState == formatedLine.blockState && prevRegion == formatedLine.regionTree)
				break;
		}
	...
	public SyntaxToken GetTokenAt(TextPosition position, out int lineIndex, out int tokenIndex, out bool atTokenEnd)
	{
		...
		tokenIndex = 0;
		if (lineIndex < 0 || lineIndex >= formatedLines.Length)
			return null;
	...
	public SyntaxToken GetNonTriviaTokenAfter(SyntaxToken token)
	{
		...
		while (lineIndex < formatedLines.Length)
		{
			var tokens = formatedLines[lineIndex].tokens;
