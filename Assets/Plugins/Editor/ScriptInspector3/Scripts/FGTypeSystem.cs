...
public class ThisReference : InstanceDefinition
{
	...
	{
		var integerLiteralType = type as IntegerLiteralType;
		if (integerLiteralType != null)
			type = integerLiteralType.TypeOf() as TypeDefinitionBase;
...
public class TypeDefinition : TypeDefinitionBase
{
	...
	private StringBuilder sbCached = new StringBuilder();
	...
	public virtual ConstructedTypeDefinition ConstructType(SymbolReference[] typeArgs)
	{
		...
		if (typeArgs != null)
		{
			...
			{
				sbCached.Append(delimiter);
				sbCached.Append(arg.ToString());
				delimiter = ", ";
			}
		}
		var sig = sbCached.ToString();
...
public class MethodGroupDefinition : SymbolDefinition
{
	...
	public static MethodDefinition ResolveMethodOverloads(int numArguments, int numCandidates)
	{
		...
		var firstNamedArgIndex = numArguments;
		while (firstNamedArgIndex > 0)
		{
			var argName = namedArgumentsStack[argsBaseIndex + firstNamedArgIndex - 1];
			if (argName == null)
				break;
			--firstNamedArgIndex;
		}
	...
	public virtual void RemoveDeclaration(SymbolDeclaration symbol)
	{
		...
		else
		{
			...
			while (index --> 0)
			{
				...
				if (x.ContainsDeclaration(symbol))
					break;
			}
			if (index >= 0)
			{
	...
	//							}
			...
			case "typeArgumentList":
				...
				if (genericMethodGroup != null)
				{
					...
					return constructedSymbolReference ?? (SymbolDefinition) genericMethodGroup;
				}
...
//					if (asMemberOf.kind != SymbolKind.Event)
			...
			case "argumentName":
				...
				var invokedNode = arguments.FindPreviousNode();
				var invokedSymbol = FGResolver.GetResolvedSymbol(invokedNode);
				...
				if (invokableSymbol != null)
					return invokableSymbol.ResolveParameterName(parameterNameLeaf);
					...
					if (selectClauseNode != null)
					{
						...
						if (selectExpressionNode != null)
						{
							...
							if (element != null)
							{
								...
								if (elementType != null)
								{
									var constructedType = builtInTypes_IEnumerable_1.ConstructType(new[]{ new SymbolReference(elementType) });
...
public class AssemblyDefinition : SymbolDefinition
{
	...
	private static List<Assembly> domainAssemblies {
		get {
			if (_domainAssemblies != null)
				return _domainAssemblies;

			_domainAssemblies = new List<Assembly>(AppDomain.CurrentDomain.GetAssemblies());
	...
	{
		_domainAssemblies.Add(args.LoadedAssembly);
	}
	...
	private static AssemblyDefinition FromPath(string assemblyPath)
	{
		...
		for (var i = domainAssemblies.Count; i --> 0; )
		{
	...
	public static AssemblyDefinition[] GetAllCSharpAssemblyDefinitions()
	{
		...
		foreach (var x in domainAssemblies)
		{
			if (!IsScriptAssembly(x))
				continue;
			var asmDef = FromAssembly(x);
